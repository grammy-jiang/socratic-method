...
def _parse_stream_json(stdout: str) -> tuple[str | None, str]:
    """Return (session_id, final_result_text) from a claude -p stream-json run."""
    session_id, result_text = None, ""
...
def _find_leaked_brief(run_started: float) -> Path | None:
    """Detect a brief the examiner wrote OUTSIDE its sandbox (repo-root notes/) —
    a harness leak that must be reported as such, never as 'no brief written'."""
    leak_dir = REPO_ROOT / "notes" / "idea-briefs"
    if not leak_dir.is_dir():
        return None
    leaked = [p for p in leak_dir.glob("*.md") if p.stat().st_mtime >= run_started]
    return sorted(leaked, key=lambda p: p.stat().st_mtime)[-1] if leaked else None
...
def run_cell(scenario: dict, args, report_dir: Path) -> dict:
    ...
    for turn in range(1, int(scenario.get("max_turns", 16)) + 1):
        session_id, examiner_text = examiner_call(workdir, args.model, prompt, session_id)
    ...
    if brief_path is None:
        leaked = _find_leaked_brief(run_started)
...
def main() -> int:
    ...
    summary_lines = [
        ...
        "",
        "| cell | name | result | graders | judge ebm |",
        "|---|---|---|---|---|",
    ]
    for r in results:
        ...
        summary_lines.append(
            ...
            f"| {graders or r.get('error', '')} | {r.get('judge_expected_behavior_met')} |"
        )
